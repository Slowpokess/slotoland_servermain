...
func InitStorage() (err error) {
	...
	for {
		...
		for _, user := range chunk {
			user.Init()
			if err = api.EnsureUserSecretHash(user); err != nil {
				return
			}
...
func SqlLoop(exitctx context.Context) {
	...
	for {
		...
		case <-passers:
			if removed, err := api.CleanupUnactivatedUsers(cfg.XormStorage, time.Now().Add(-time.Hour*3*24)); err != nil {
				log.Printf("can not cleanup inactive users: %s", err.Error())
			} else if removed > 0 {
				log.Printf("cleaned up %d inactive users", removed)
			}
		case <-exitctx.Done():
			return
		}
