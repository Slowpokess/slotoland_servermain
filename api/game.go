...
// Returns formatted list of all games for frontend gallery.
func ApiGameInfoPublic(c *gin.Context) {
	...
	games := make([]GamePublic, 0)

	for _, alg := range game.AlgList {
		for _, alias := range alg.Aliases {
			g := GamePublic{
				ID:       alias.Prov + "/" + alias.Name,
				Name:     alias.Name,
				Provider: alias.Prov,
				Algo:     fmt.Sprintf("%p", alg), // Use algorithm pointer as ID
				RTP:      alg.RTP,
				Reels:    alg.SX,
				Rows:     alg.SY,
				Lines:    alg.LN,
			...
			// Volatility based on RTP
			if len(alg.RTP) > 0 {
				avgRTP := 0.0
				for _, r := range alg.RTP {
					avgRTP += r
				}
				avgRTP /= float64(len(alg.RTP))
			...
			// Payment type
			if gp&game.GPlpay > 0 {
				props = append(props, "Left-to-right")
			}
			if gp&game.GPrpay > 0 {
				props = append(props, "Both ways")
			}
			if gp&game.GPcpay > 0 {
				props = append(props, "Cluster pay")
			}
			if gp&game.FPspay > 0 {
				props = append(props, "Scatter pay")
			}

			// Features
			if gp&game.GPjack > 0 {
				props = append(props, "Jackpot")
			}
			if gp&game.GPfill > 0 {
				props = append(props, "Full screen multiplier")
			}
			if gp&game.GPcasc > 0 {
				props = append(props, "Cascade")
			}
			if gp&game.GPfghas > 0 || gp&game.GPretrig > 0 {
				props = append(props, "Free games")
			}
			if gp&game.GPrwild > 0 {
				props = append(props, "Reel wild")
			}
			if gp&game.GPbwild > 0 {
				props = append(props, "Big wild")
			}
			if gp&game.GPwild > 0 {
				props = append(props, "Wild")
			}
			if gp&game.GPscat > 0 {
				props = append(props, "Scatter")
			}
			if gp&game.GPrmult > 0 {
				props = append(props, "Multipliers")
			}
...
func ApiGameNew(c *gin.Context) {
	...
	var ret struct {
		...
		State   SceneState `json:"state" yaml:"state" xml:"state"`
...
func ApiGameJoin(c *gin.Context) {
	...
	scene.Lock()
	defer scene.Unlock()
	if scene.UID != arg.UID || scene.CID != arg.CID {
		Ret403(c, AEC_game_join_noaccess, ErrNoAccess)
		return
	}
...
func ApiGameClose(c *gin.Context) {
	var err error
	var ok bool
	var arg struct {
		XMLName xml.Name `json:"-" yaml:"-" xml:"arg"`
	...
	if err = c.ShouldBind(&arg); err != nil {
		Ret400(c, AEC_game_close_nobind, err)
		return
	}

	var scene *Scene
	if scene, err = GetScene(arg.GID); err != nil {
		Ret404(c, AEC_game_close_noscene, err)
		return
	}
	scene.Lock()
	defer scene.Unlock()
	...
	if user, ok = Users.Get(scene.UID); !ok {
		Ret500(c, AEC_game_close_nouser, ErrNoUser)
		return
	}

	var admin, al = MustAdmin(c, scene.CID)
	if admin != user && al&ALdealer == 0 {
		Ret403(c, AEC_game_close_noaccess, ErrNoAccess)
		return
	}

	closedAt := scene.XTime
	if closedAt.IsZero() {
		closedAt = time.Now()
	}
	if _, err = cfg.XormStorage.ID(scene.GID).Cols("closed", "xtime").Update(&Story{
		Closed: true,
		XTime:  closedAt,
	}); err != nil {
		Ret500(c, AEC_game_close_sql, err)
		return
	}
	scene.CloseAt(closedAt)
