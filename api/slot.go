...
func ApiSlotBetGet(c *gin.Context) {
	...
	scene.Lock()
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_betget_noaccess, err)
		return
	}
...
func ApiSlotBetSet(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_betset_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
...
func ApiSlotSelGet(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_selget_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
...
func ApiSlotSelSet(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_selset_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
...
func ApiSlotModeSet(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_modeset_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
...
func ApiSlotSpin(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_spin_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
	...
	// write spin settlement as transaction
	var sid = atomic.AddUint64(&SpinCounter, 1)
	var jprent = cost * jprate / 100
	var wallet float64
	if wallet, err = ApplyGameplayWalletSettlement(cfg.XormStorage, GameplayWalletSettlement{
		CID:      scene.CID,
		UID:      scene.UID,
		ActorUID: admin.UID,
		GID:      arg.GID,
...
func ApiSlotDoubleup(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_doubleup_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
	...
	// write double-up settlement as transaction
	var wallet float64
	if wallet, err = ApplyGameplayWalletSettlement(cfg.XormStorage, GameplayWalletSettlement{
		CID:      scene.CID,
		UID:      scene.UID,
		...
		if err == ErrNoMoney {
			Ret403(c, AEC_slot_doubleup_nogain, ErrNoMoney)
			return
		}
...
func ApiSlotCollect(c *gin.Context) {
	...
	defer scene.Unlock()
	if err = scene.EnsureOpen(); err != nil {
		Ret403(c, AEC_slot_collect_noaccess, err)
		return
	}
	var game slot.SlotGame
	if game, ok = scene.Game.(slot.SlotGame); !ok {
