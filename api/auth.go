...
var (
	...
	ErrBadUse   = errors.New("jwt-token has unexpected use")
	ErrNoLegacy = errors.New("nonce-based sign-in is unavailable with secure password storage; send secret over TLS")
...
var RefreshAuthGetters = []AuthGetter{
	RefreshUserFromHeader,
}
...
func Auth(required bool) gin.HandlerFunc {
	return AuthWith(required, AuthGetters)
}

func AuthWith(required bool, getters []AuthGetter) gin.HandlerFunc {
...
func RefreshAuth(required bool) gin.HandlerFunc {
	return AuthWith(required, RefreshAuthGetters)
}
...
// RequireBackoffice permits the read-only support role and higher roles.
func RequireBackoffice() gin.HandlerFunc {
	return func(c *gin.Context) {
		_, al := MustAdmin(c, 0)
		if !al.CanAccessBackoffice() {
			Ret403(c, AEC_auth_admin, ErrNoAccess)
			return
		}
		c.Next()
...
// RequireOperator permits operational and admin actions, but not support-only users.
func RequireOperator() gin.HandlerFunc {
	return func(c *gin.Context) {
		_, al := MustAdmin(c, 0)
		if role := al.BackofficeRole(); role != BackofficeRoleOperator && role != BackofficeRoleAdmin {
			Ret403(c, AEC_auth_admin, ErrNoAccess)
			return
		}
		c.Next()
...
func RefreshUserFromHeader(c *gin.Context) (*User, int, error) {
	if hdr := c.Request.Header.Get("Authorization"); hdr != "" {
		if strings.HasPrefix(hdr, "Bearer ") {
			return GetRefreshAuth(hdr[7:])
		}
		return nil, AEC_auth_scheme, ErrNoScheme
	}
	return nil, 0, nil
...
func RefreshUserFromQuery(c *gin.Context) (*User, int, error) {
	if tokenstr := c.Query("token"); tokenstr != "" {
		return GetRefreshAuth(tokenstr)
	} else if tokenstr := c.Query("jwt"); tokenstr != "" {
		return GetRefreshAuth(tokenstr)
	}
	return nil, 0, nil
...
func RefreshUserFromCookie(c *gin.Context) (*User, int, error) {
	if tokenstr, _ := c.Cookie("token"); tokenstr != "" {
		return GetRefreshAuth(tokenstr)
	} else if tokenstr, _ := c.Cookie("jwt"); tokenstr != "" {
		return GetRefreshAuth(tokenstr)
	}
...
func GetBasicAuth(credentials string) (user *User, code int, err error) {
	...
	if len(parts) != 2 {
		return nil, AEC_basic_decode, ErrNoCred
	}
	...
	if user.Status&UFblocked != 0 {
		err, code = ErrBlocked, AEC_auth_blocked
		return
	}
	if !VerifySecret(user.Secret, parts[1]) {
...
func GetBearerAuth(tokenstr string) (user *User, code int, err error) {
	return getTokenAuth(tokenstr, tokenUseAccess, []byte(Cfg.AccessKey))
}

func GetRefreshAuth(tokenstr string) (user *User, code int, err error) {
	return getTokenAuth(tokenstr, tokenUseRefresh, []byte(Cfg.RefreshKey))
}

func getTokenAuth(tokenstr, wantUse string, key []byte) (user *User, code int, err error) {
	var claims Claims
	...
	if claims.Use != wantUse {
		return nil, AEC_token_badclaims, ErrBadUse
	}
	var ok bool
	if user, ok = Users.Get(claims.UID); !ok {
		return nil, AEC_token_nouser, ErrBadJwtID
	}
	if user.Status&UFblocked != 0 {
		return nil, AEC_auth_blocked, ErrBlocked
	}
	return user, 0, nil
}
...
type AuthResp struct {
	...
	Refresh string   `json:"refresh" yaml:"refresh" xml:"refresh"`
	Refrsh  string   `json:"refrsh" yaml:"refrsh" xml:"refrsh"` // deprecated compatibility alias
...
func (r *AuthResp) Setup(user *User) {
	...
	if r.Refresh, err = token.SignedString([]byte(Cfg.RefreshKey)); err != nil {
		panic(err)
	}
	r.Refrsh = r.Refresh
...
func ApiSignup(c *gin.Context) {
	...
	if user.Secret, err = HashSecret(arg.Secret); err != nil {
...
func ApiSignin(c *gin.Context) {
	...
	if user.Status&UFblocked != 0 {
		Ret403(c, AEC_auth_blocked, ErrBlocked)
		return
	}
	...
	if len(arg.Secret) > 0 {
		if !VerifySecret(user.Secret, arg.Secret) {
			Ret403(c, AEC_signin_denypass, ErrNotPass)
			return
		}
	} else {
		if SecretIsHashed(user.Secret) {
			Ret400(c, AEC_signin_legacy, ErrNoLegacy)
			return
		}
